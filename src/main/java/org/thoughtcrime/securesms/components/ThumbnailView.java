...
public class ThumbnailView extends FrameLayout {
  ...
  public ThumbnailView(final Context context, AttributeSet attrs, int defStyle) {
    ...
    if (attrs != null) {
      try (TypedArray typedArray =
        ...
        // int radius = typedArray.getDimensionPixelSize(R.styleable.ThumbnailView_thumbnail_radius,
        // getResources().getDimensionPixelSize(R.dimen.gallery_thumbnail_radius));
      }
    }
  }
  ...
  private void fillTargetDimensions(int[] targetDimens, int[] dimens, int[] bounds) {
    ...
    if (dimensFilledCount > 0 && dimensFilledCount < dimens.length) {
      throw new IllegalStateException(
          String.format(
              Locale.ENGLISH,
              "Width or height has been specified, but not both. Dimens: %f x %f",
              naturalWidth,
              naturalHeight));
    }
  ...
  public ListenableFuture<Boolean> setImageResource(
      @NonNull GlideRequests glideRequests, @NonNull Slide slide) {
  ...
  public ListenableFuture<Boolean> setImageResource(
    ...
    if (this.slide != null
        && this.slide.getFastPreflightId() != null
    ...
    Log.w(
        ...
        "loading part with id "
            + slide.asAttachment().getDataUri()
            + ", progress "
            + slide.getTransferState()
            + ", fast preflight id: "
