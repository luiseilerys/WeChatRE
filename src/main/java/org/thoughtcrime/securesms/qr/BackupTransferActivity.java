...
public class BackupTransferActivity extends BaseActionBarActivity {
  ...
  protected void onCreate(Bundle icicle) {
    ...
    transferMode =
        TransferMode.fromInt(getIntent().getIntExtra(TRANSFER_MODE, TransferMode.INVALID.getInt()));
    ...
    String title =
        getString(
            transferMode == TransferMode.RECEIVER_SCAN_QR
                ? R.string.multidevice_receiver_title
                : R.string.multidevice_title);
  ...
  public void doFinish() {
    // the permanent notification will prevent other activities to be started and kill
    // BackupTransferActivity;
    // close it before starting other activities
    notificationController.close();
    notificationControllerClosed = true;

    if (transferMode == TransferMode.RECEIVER_SCAN_QR
      ...
      // (we removed the activity backstack as otherwise a tap on the ArcaneChat icon on the home
      // screen would
      ...
      new Thread(
              () -> {
                try {
                  ...
                  // ACCESS_WIFI_STATE, ACCESS_NETWORK_STATE and maybe even more.
                  ...
                  if (wifiManager.isWifiEnabled()) {
                    ...
                    if (!ssid.equals(
                        "<unknown ssid>")) { // "<unknown ssid>" may be returned on insufficient
                      // rights
                      Util.runOnMain(
                          () -> {
