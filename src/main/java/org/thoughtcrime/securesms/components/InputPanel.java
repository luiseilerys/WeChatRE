...
public class InputPanel extends ConstraintLayout
  ...
  public void setQuote(
    ...
    int originalHeight =
        this.quoteView.getVisibility() == VISIBLE ? this.quoteView.getMeasuredHeight() : 0;
  ...
  private static ValueAnimator createHeightAnimator(
      @NonNull View view,
      int originalHeight,
      int finalHeight,
      @Nullable AnimationCompleteListener onAnimationComplete) {
    ValueAnimator animator = ValueAnimator.ofInt(originalHeight, finalHeight).setDuration(200);
  ...
  public void setSubjectVisible(boolean visible) {
    ...
    // don't make it visible if visible is false to avoid showing it while recording audio and an
    // event triggers setSubjectVisible(false)
    ...
    public ListenableFuture<Void> hide() {
      ...
      animation.addAnimation(
          new TranslateAnimation(
              Animation.ABSOLUTE, slideToCancelView.getTranslationX(),
      ...
      slideToCancelView.postDelayed(
          () -> future.set(null), MicrophoneRecorderView.ANIMATION_DURATION);
